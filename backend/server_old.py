...
# Create Flask app (pure API mode)
app = Flask(__name__)
...
def handle_terminal_run(data):
    ...
    try:
        ...
        # On Windows, we force UTF-8 for both Java and the pipe communication
        proc = subprocess.Popen(
            [JAVA_PATH, "-Dfile.encoding=UTF-8", "-Dsun.stdout.encoding=UTF-8", "-Dsun.stderr.encoding=UTF-8", "-cp", temp_dir, "Main"],
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
            text=True,           # Use text mode with explicit encoding
            encoding='utf-8',
            errors='replace',
            bufsize=1,           # Line buffered for smoother interaction
            cwd=temp_dir,
        )
        ...
        def _stream_output():
            try:
                # Use read() with a small buffer to capture prompts without newlines
                while True:
...
def handle_terminal_input(data):
    ...
    if proc and proc.poll() is None:
        try:
            ...
            # If proc is in text mode (text=True), write takes a str
...
def visualize_endpoint():
    """Visualize Java execution steps"""
    try:
        data = request.get_json()
        code = data.get("code", "")
        if not code:
        ...
        from visualizer import visualize_code
